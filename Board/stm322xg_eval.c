...
/** * @defgroup STM322xG_EVAL_LOW_LEVEL 
	...
	*/
uint32_t m_uCanRecvID; //used as a flag indicate can data has been received.
...
BTN_PARAM BTNs[] = {
	{GPIOA, RCC_AHB1Periph_GPIOA, GPIO_Pin_3,  EXTI_Line3,  EXTI_PortSourceGPIOA, EXTI_PinSource3,  1, EXTI3_IRQn},	//Unknown push-button
	...
	{GPIOF, RCC_AHB1Periph_GPIOF, GPIO_Pin_15, EXTI_Line15, EXTI_PortSourceGPIOF, EXTI_PinSource15, 1, EXTI15_10_IRQn},	//Key push-button
	{NULL}
};
BTN_PARAM JOYs[] = {
...
CAN_PARAM CANs[] = {
	...
	{CAN1, RCC_APB1Periph_CAN1, GPIO_AF_CAN1, RCC_AHB1Periph_GPIOD, GPIOD, GPIO_Pin_0, GPIO_PinSource0, GPIO_Pin_1, GPIO_PinSource1, 
		CAN1_RX0_IRQn, CAN1_RX1_IRQn, CAN_BS1_14tq, CAN_BS1_5tq, 5},
...
TIM_PARAM TIMs[] = {
	{TIM1, 0x00},
	{TIM2, 0x00},
	{TIM3, RCC_APB1Periph_TIM3, 1200, TIM_CounterMode_Up, 1200-1,	TIM_OCMode_Timing, TIM_FLAG_Update, TIM_IT_Update, TIM3_IRQn},
	{TIM4, RCC_APB1Periph_TIM4, 1200, TIM_CounterMode_Up, 90-1,		TIM_OCMode_Timing, TIM_FLAG_Update, TIM_IT_Update, TIM4_IRQn},
	{TIM5, 0x00},
	{TIM6, 0x00},
	{TIM7, 0x00},
	{TIM8, 0x00},
...
{ 
	...
	LONG i = 0;
	while (TIMs[i].timx != NULL)
	{
		if (TIMs[i].clock == 0x00)
		{
			i ++;
			continue;
		}
		/* Enable TIMx clocks */
		RCC_APB1PeriphClockCmd(TIMs[i].clock, ENABLE);

		/* TIMx configuration */        
		tim_init.TIM_Prescaler = ((SystemCoreClock / TIMs[i].scaler) - 1);
		tim_init.TIM_ClockDivision = 0x0;    
		tim_init.TIM_CounterMode = TIMs[i].cmode;  
		tim_init.TIM_Period = TIMs[i].period;  
		TIM_TimeBaseInit(TIMs[i].timx, &tim_init);
		
		/* Output Compare Timing Mode configuration: Channel1 */
		TIM_OCStructInit(&tim_oc);
		tim_oc.TIM_OCMode = TIMs[i].ocmode;
		tim_oc.TIM_Pulse = 0x0;
		TIM_OC1Init(TIMs[i].timx, &tim_oc);
		
		/* Immediate load of Precaler values */
		TIM_PrescalerConfig(TIMs[i].timx, ((SystemCoreClock / TIMs[i].scaler) - 1), TIM_PSCReloadMode_Immediate);
		/* Clear TIMx update pending flags */
		TIM_ClearFlag(TIMs[i].timx, TIMs[i].tflag);
		...
		TIM_ITConfig(TIMs[i].timx, TIMs[i].isrc, ENABLE);
		/* TIMx enable counters */
		TIM_Cmd(TIMs[i].timx, ENABLE);
		i ++;
	}
}
...
{
	while (CAN_GetFlagStatus(CAN1, CAN_FLAG_FMP0) != SET && tmout > 0)
	{
		Sleep(1);
		tmout -= 1;
	}
