/**
	...
	*				while(SD_GetStatus() != SD_TRANSFER_IDLE);
	*
...
void SD_NVIC_Config(void)
{
	...
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);//0:4
...
void SD_LowLevel_Init(void)
{
	...
	gpio_init.GPIO_Pin = SD_DETECT_PIN;
	gpio_init.GPIO_Mode = GPIO_Mode_IN;
	gpio_init.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(SD_DETECT_GPIO_PORT, &gpio_init);
...
void SD_LowLevel_DMA_TxConfig(uint32_t *src, uint32_t size)
{
	...
	SDDMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)src;
	...
	DMA_ITConfig(SDIO_DMA_STREAM, DMA_IT_TC, ENABLE);//Transfer complete interrupt
...
void SD_LowLevel_DMA_RxConfig(uint32_t *dst, uint32_t size)
{
	...
	SDDMA_InitStructure.DMA_Channel = SDIO_DMA_CHANNEL;
	SDDMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;
	SDDMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)dst;
...
SD_Error SD_Init(void)
{
	...
	//(SD Card can not be initialized with this portion, don't know why.)
...
/**	* @brief	Gets the cuurent sd card data transfer status.
	...
	*	This value can be: 
	*			- SD_TRANSFER_IDLE: No data transfer is acting
	*			- SD_TRANSFER_BUSY: Data transfer is acting
	*/
SDTransferState SD_GetTransStatus(void)
{
	SDCardState cardstate =	SD_CARD_TRANSFER;
	cardstate = SD_GetCardState();

	if (cardstate == SD_CARD_TRANSFER)
		return(SD_TRANSFER_IDLE);
...
/**	* @brief	Returns the current card's state.
	...
	*/
SDCardState SD_GetCardState(void)
{
	uint32_t resp1 = 0;
...
SD_Error SD_ReadBlock(uint32_t ReadAddr, uint8_t* readbuff, uint16_t BlockSize)
{
	...
	SDIO_ClearFlag(SDIO_STATIC_FLAGS);
	...
	#elif defined (SD_INTERRUPT_MODE)
		...
		SDIO_DMACmd(ENABLE);
		SD_LowLevel_DMA_RxConfig((uint32_t *)readbuff, BlockSize);
		#ifndef _SD_WAIT_DMA//(wait for complete here rather than by dma)
