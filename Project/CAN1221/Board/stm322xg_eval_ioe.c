...
void sEE_LowLevel_DeInit(void)
{
	...
	gpio_init.GPIO_Pin = sEE_I2C_SCL_PIN;
	gpio_init.GPIO_Mode = GPIO_Mode_IN;
	gpio_init.GPIO_PuPd = GPIO_PuPd_NOPULL;
	GPIO_Init(sEE_I2C_SCL_GPIO_PORT, &gpio_init);

	/*!< Configure sEE_I2C pins: SDA */
	gpio_init.GPIO_Pin = sEE_I2C_SDA_PIN;
	GPIO_Init(sEE_I2C_SDA_GPIO_PORT, &gpio_init);

	nvic_init.NVIC_IRQChannelPreemptionPriority = sEE_I2C_DMA_PREPRIO;
	nvic_init.NVIC_IRQChannelSubPriority = sEE_I2C_DMA_SUBPRIO;
	nvic_init.NVIC_IRQChannelCmd = DISABLE;
	/* Configure and enable I2C DMA TX Stream interrupt */
	nvic_init.NVIC_IRQChannel = sEE_I2C_DMA_TX_IRQn;
	NVIC_Init(&nvic_init);
	/* Configure and enable I2C DMA RX Stream interrupt */
	nvic_init.NVIC_IRQChannel = sEE_I2C_DMA_RX_IRQn;
	NVIC_Init(&nvic_init);	 
...
void sEE_LowLevel_Init(void)
{
	...
	DMA_DeInit(sEE_I2C_DMA_STREAM_TX);
	eedma_init.DMA_Channel = sEE_I2C_DMA_CHANNEL;
	eedma_init.DMA_PeripheralBaseAddr = (uint32_t)sEE_I2C_DR_Address;
	eedma_init.DMA_Memory0BaseAddr = (uint32_t)0;		/* This parameter will be configured durig communication */;
	eedma_init.DMA_DIR = DMA_DIR_MemoryToPeripheral; /* This parameter will be configured durig communication */
	eedma_init.DMA_BufferSize = 0xFFFF;							/* This parameter will be configured durig communication */
	eedma_init.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	eedma_init.DMA_MemoryInc = DMA_MemoryInc_Enable;
	eedma_init.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
	eedma_init.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
	eedma_init.DMA_Mode = DMA_Mode_Normal;
	eedma_init.DMA_Priority = DMA_Priority_VeryHigh;
	eedma_init.DMA_FIFOMode = DMA_FIFOMode_Enable;
	eedma_init.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;
	eedma_init.DMA_MemoryBurst = DMA_MemoryBurst_Single;
	eedma_init.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;
	DMA_Init(sEE_I2C_DMA_STREAM_TX, &eedma_init);
	...
	DMA_DeInit(sEE_I2C_DMA_STREAM_RX);
	DMA_Init(sEE_I2C_DMA_STREAM_RX, &eedma_init);
...
/**	* @brief	Initializes DMA channel used by the I2C EEPROM driver.
	...
	* @retval 	None
*/
void sEE_LowLevel_DMAConfig(uint32_t buff, uint32_t size, uint32_t dir)
{
	DMA_InitTypeDef		eedma_init; 
	eedma_init.DMA_Channel = sEE_I2C_DMA_CHANNEL;
	eedma_init.DMA_PeripheralBaseAddr = (uint32_t)sEE_I2C_DR_Address;
	...
	eedma_init.DMA_Memory0BaseAddr = (uint32_t)buff;
	eedma_init.DMA_BufferSize = (uint32_t)size;
	if (dir == sEE_DIRECTION_TX)
	{
		/* Configure the DMA Tx Stream with the buffer address and the buffer size */
		eedma_init.DMA_DIR = DMA_DIR_MemoryToPeripheral;
		DMA_Init(sEE_I2C_DMA_STREAM_TX, &eedma_init);	
	}
	else
	{ 
		/* Configure the DMA Rx Stream with the buffer address and the buffer size */
		eedma_init.DMA_DIR = DMA_DIR_PeripheralToMemory;
		DMA_Init(sEE_I2C_DMA_STREAM_RX, &eedma_init);		
	}
}
...
/**
  ...
  *   @arg  JOY_UP
  */
JOY_INDEX IOE_JoyStickGetState(void)
{
  uint8_t tmp = 0;
